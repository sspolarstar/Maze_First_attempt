...
void draw_text(bool i_center, unsigned short i_x, unsigned short i_y, const std::string& i_text, sf::RenderWindow& i_window) {
	...
	if (1 == i_center)
	{
		...
		character_y = static_cast<short>(round(0.5f * (CELL_SIZE * MAP_HEIGHT - TEXT_HEIGHT * (1 + std::count(i_text.begin(), i_text.end(), '\n')))));
	}
	...
	for (std::string::const_iterator a = i_text.begin(); a != i_text.end(); a++)
	{
		...
		character_sprite.setTextureRect(sf::IntRect(character_width * (*a - 32), 0, character_width, TEXT_HEIGHT));
